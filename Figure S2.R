...
plot_data2 <- k_data %>%
  group_by(mid_depth) %>%
  summarise(mean = mean(ksat, na.rm = TRUE),
            sd = sd(ksat, na.rm = TRUE),
            n = n(),
            max = max(ksat, na.rm = TRUE),
            min = min(ksat, na.rm = TRUE)) %>%
  ungroup()
...
ggsave(plot = p3, filename = "Figure S2.png", height = 170, width = 174, units = "mm", dpi = 600)
