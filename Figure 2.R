...
model_data <- data %>%
  ...
  pivot_longer(cols = c("Model 4", "Model 2", "Model 3", "Model 1"), names_to = "Model", values_to = "K_sat") %>%
...
k_plot <- ggplot(data = filter(plot_data, Model != "Measured"), aes(x = mid_depth, y = K_sat, colour = Model)) +
  #geom_point() +
  geom_line(linewidth = 0.5) +
  geom_point(data = filter(plot_data, Model == "Measured"), aes(x = mid_depth, y = K_sat)) +
  geom_line(data = filter(plot_data, Model == "Measured"), aes(x = mid_depth, y = K_sat)) +
  theme_bw() +
